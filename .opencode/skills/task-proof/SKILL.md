---
name: task-proof
description: Run a proof loop for complex OpenCode tasks: freeze acceptance criteria, build, capture evidence, run an independent verifier, and iterate on failures.
---

# Task Proof Loop

Use this skill for complex, multi-file OpenCode work where a normal final summary is not enough proof: freeze spec, build, gather evidence, fresh-verify, fix, and report.

**Trigger:** The user invokes `task-proof` explicitly, or task-proof's prompt nudge recommends it for a complex request.

**Scope boundary:** This skill manages verification artifacts and the proof lifecycle. It does not replace OpenCode permissions, project rules, or other installed plugins.

## Agentic Protocol

- Track the seven steps with OpenCode's task-tracking tool and keep statuses current.
...
Small skill improvements belong in `adapters/opencode/skills/task-proof/SKILL.md` in the task-proof repo. Structural changes should be proposed first.
